...
function validate_delivery($r_f_no, $r_kg, $r_dt) {
    ...
    if ($r_kg == '') {
        $has_errors = TRUE;
        $errors['kg'] = '<span class="label label-warning "><i class="icon-warning-sign icon-white"></i> Enter Number of Litres Delivered!</span>';
    } elseif (!is_numeric($r_kg)) {
        $has_errors = TRUE;
        $errors['kg'] = '<span class="label label-warning "><i class="icon-warning-sign icon-white"></i> Enter Litres in numbers!</span>';
    }
    if ($r_dt=='') {
        $has_errors = TRUE;
